...
if(loadProjectBtn && projectLoader) loadProjectBtn.addEventListener("click", () => {
  projectLoader.click();
});

if(projectLoader) projectLoader.addEventListener("change", async (e) => {
...
async function createNewFileFlow(){
  ...
  // Use the normalized path (rebuilt from the parsed parts) everywhere below,
  // not the raw prompt() text — the tree view always renders paths in this
  // normalized form, so storing/selecting under the raw text can leave the
  // new file's content keyed differently than what the tree looks up,
  // making it appear empty once you click away and back.
  ...
  if(linkedProjectDirHandle){
    ...
    if(ok){
      try{
        await writeFileToDisk(linkedProjectDirHandle, normalizedPath, "");
        lastSyncedSnapshot = { ...fileContents };
      }catch(err){
  ...
  rebuildWorkspaceView();
  showToast(`Created file "${normalizedPath}"`);
}
...
async function createNewFolderFlow(){
  ...
  rebuildWorkspaceView();
  showToast(`Created folder "${normalizedPath}"`);
}
